...
def OfflineStructredasHetteroAssociatedSequenceLerninginCa3(): # when Ec.cells == Ca3.cells >> I am testing for the same weight matrics in Feed forward and recurrent colaterals!!!!
#if __name__ == "__main__":
    ...
    noise_levels_ca3= np.arange(0, 1101, 100)  
    plastic= 1
    centers = np.ravel((np.mgrid[0:50, 0:50] + 0.0), order = 'F').reshape(1, 2500, 1, 2)/50.
...
##########################
    ...
    #Ca3 = Grid(number_cells = 2500, noiseMethod = Input.makeNoiseRandomFire, actFunction = Input.getOutputWTA, number_patterns = num_all_pat, number_to_store =num_all_pat ,n_e =1,noise_levels = noise_levels_ca3, store_indizes = store_indizes, grid_mode = 'modules', cage =[1,1], sparsity = 0.35)# actFunction?
...
#################################################################
    ...
    np.savetxt('OfflineStructredConnections.txt', Ca3_Ca3.connection)
    
    return ca3_input, Ca3.noisy_input_stored[:,0,np.arange(0, num_all_pat-seq_len +1, seq_len)], Ca3.noisy_input_stored[:,1,np.arange(0, num_all_pat-seq_len +1, seq_len)], Ca3.noisy_input_stored[:,2,np.arange(0, num_all_pat-seq_len +1, seq_len)]



#if __name__ == "__main__":

    #RGSTP(initConnectionMethod= Network.initConnectionNEST, initMethod = Network.makeWeightsOne, in_weight= 0.0185, plastic= 0)
    #RGSTP(initConnectionMethod= Network.initConnectionRandom, initMethod = Network.makeWeightsexponential, in_weight= 0.85 , plastic= 0)
    ##RGSTP(initConnectionMethod= Network.initConnectionRandom, initMethod = Network.makeWeightsNormalDistributed, in_weight= 0.175, plastic= False)
    ##store_indizes = np.arange(256).reshape(1,256)
    ##In = Grid(number_cells = 1100, noiseMethod = Input.makeNoiseZero, actFunction = Input.getOutputId, number_patterns = num_all_pat, number_to_store =256 ,n_e =1,noise_levels = np.arange(0, 1101, 50), store_indizes = store_indizes, grid_mode = 'modules', cage =[1,1], sparsity = 0.35)
    #mainExperiment()    
...
if __name__ == "__main__":
    ...
    seq_len = 400
    num_all_pat = (num_seq * seq_len)
    in_weight = .636  ### it is not working when it is exactly zero
    noise_levels= np.arange(0, 1101, 350)
    plastic= 0
    #############################################################      
    
    allcorrdata = np.zeros([2, 4, 3, len(noise_levels), num_seq, seq_len]) 
    ...
    for inputstatistics in range(1,2):
        ...
        for ca3models in range(2,3):
            if ca3models == 0:          
                ca3_model_random=1; ca3_model_local=0;  ca3_model_pretrained=0; ca3_model_onlinetraining=0
                ca3modeltitle = "RCRN model, "
            if ca3models == 1:          
                ca3_model_random=0; ca3_model_local=1;  ca3_model_pretrained=0; ca3_model_onlinetraining=0
                ca3modeltitle = "LCRN model, "
                #in_weight = .76 
            if ca3models == 2:          
                ca3_model_random=0; ca3_model_local=0;  ca3_model_pretrained=1; ca3_model_onlinetraining=0
                ca3modeltitle = "Pretrained model, "
            if ca3models == 3:          
                ca3_model_random=0; ca3_model_local=0;  ca3_model_pretrained=0; ca3_model_onlinetraining=1
                ca3modeltitle = "Trained via Ec input, "                                                
        ...
        #################################################################
            ...
            if ca3_model_local:
                ...
                EcCa3initMethod = Network.makeWeightsZero
                originalSeqs, Ca3_original_noisy_0_input_stored , Ca3_original_noisy_1_input_stored, Ca3_original_noisy_2_input_stored = OfflineStructredasHetteroAssociatedSequenceLerninginCa3()
                ...
                Ca3 = Input(inputMethod = Input.makeInputNormalDistributed,number_cells = 2500, number_patterns = num_all_pat, number_to_store = num_all_pat, actFunction = Input.getOutputWTA, noiseMethod = Input.makeNoiseRandomFire, noise_levels = noise_levels, sparsity = 0.053, store_indizes = store_indizes)        
                plastic =1  
                EcCa3initMethod = Network.makeWeightsNormalDistributed
        ...
        #################################################################
            ...
            if ca3_model_random or ca3_model_local:
                initializing_ca3_storing = Ca3.input_stored         
                #initializing_ca3_retrieving = np.copy(Ec_Ca3.noisy_output) # Ca3.noisy_input_stored#
            
            elif ca3_model_pretrained:
                initializing_ca3_storing = Ca3_original_noisy_0_input_stored
                #initializing_ca3_retrieving = Ec_Ca3.noisy_output
                
            elif ca3_model_onlinetraining:
                ca3_triggered_patterns = Ec_Ca3.getOutput(Ec_Ca3, input_pattern = Ec.input_stored)
                initializing_ca3_storing  = ca3_triggered_patterns
                #initializing_ca3_retrieving = Ec_Ca3.noisy_output 
            ...
            if ca3_model_random or ca3_model_local or ca3_model_pretrained:
                for j in range(num_seq):
                    inhibition = np.copy(initializing_ca3_storing[0,j]) 
                    self_inhibition = np.ones(initializing_ca3_storing[0,j].shape)
                    ca3_input[0, j*seq_len] =  initializing_ca3_storing[0,j]
                ...
                ca3_input=  initializing_ca3_storing
                #Ca3_Ca3.learnOneShootAllPattern(input_pattern = ca3_input, method = OneShootmehdi.learnFiringDependent, seq_len = seq_len, num_seq= num_seq) 
                for j in range(num_seq):
        ...
        ##################################################################
            ...
            Ca3_Ca1.recall(input_pattern = Ec_Ca3.noisy_output) #ca1_noisy_input_pattern)
        ...
        ###########################################################
            ...
            centers = np.ravel((np.mgrid[0:20, 0:20] + 0.0), order = 'F').reshape(400, 2)/20.
            for j in range(30):
                fig =plt.figure()
                ax = fig.add_subplot(1,1,1)
                ax.scatter(centers[:,0], centers[:,1], s = Ec_Ca3.noisy_output[0,0,:,j*13], color= 'r' ) edgecolors ='none', cmap = 'YlOrRd')
            plt.show()
